...
## dispatch prompt 定型（手順 3 で各視点 subagent に送る文面の骨格。REQUIRED を全て埋めて送信する）

```text
あなたは <対象（PR 番号/パス/diff 取得コマンド）> を「<視点名>」レンズのみでレビューする独立 subagent です。
他視点の結論は与えられていない。他観点への言及は不要（別レビュアーが担当）。
レンズ: <上の「視点」表から当該視点の定義を転記>

REQUIRED-1 実挙動検証: 主張する前に実挙動で検証せよ — spec 実行・`RAILS_ENV=test bin/rails runner`・gem ソース確認。
机上推論だけの High 以上の指摘は禁止。検証不能な主張は確信度を明示せよ。
REQUIRED-2 原典検証: 事実主張（「既存」「終端」「実在する」等）は SPEC 原文/実コードで検証し、
各指摘に Confirmed（実コードで確認済）/ Plausible（要追検証）を明記せよ。
REQUIRED-3 罠台帳: docs/RAILS_GOTCHAS.md の関連節を読み、既知の罠の再発をレンズに含めよ。
REQUIRED-4 出力: 各指摘 = file:line + 要約 + 具体的な失敗シナリオ + 修正案 + Confirmed/Plausible。重大度順。
問題ゼロなら「確認した観点と安全と判断した根拠」を列挙せよ。
```

（REQUIRED-1 は DEVELOPMENT_WORKFLOW「効率化 5 項目」③の定型句。スキル本文しか読まない controller が
実挙動検証義務と罠台帳注入を毎回落とすことが実測されたため、定型に埋め込んで構造的に運ぶ — 2026-07-07 Sonnet 2/2 反復で欠落）
