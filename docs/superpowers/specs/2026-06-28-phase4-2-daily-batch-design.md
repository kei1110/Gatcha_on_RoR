...
> **訂正注記（2026-07-09・4-2c-2 labor-law レビューの原典照合）:** 本節の「適正手続き」を労基法 24 条に帰する読みは誤り。**24 条は「通貨・直接・全額払い」「毎月一回以上一定期日払い」のみを命じ、予告・弁明機会・適正手続きの規律を含まない**。24 条が効くのは「誤確定 → 実労働日の賃金不払い」という結果側の経路であり、事前通知・猶予はその予防措置。また「賃金控除」の語も、24 条 1 項ただし書きの「控除」（労使協定を要件とする賃金からの控除）とは別概念（欠勤による不支給は通説上ノーワーク・ノーペイ＝賃金債権の不発生）。正本は `docs/SPEC.md` §6.8 / §6.10 と `docs/LABOR_LAW_REVIEW_NOTES.md` #22〜#26。本設計書は凍結資料ゆえ本文は書き換えず注記のみとする。
