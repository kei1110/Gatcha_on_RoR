...
- [ ] **3-3 CSV 2 種**: 月次サマリ・日別明細（UTF-8 BOM・割増区分網羅・§6.4）。**3-3a 休暇集計の素材整備** done（AR `leave_type_id`＝複合 FK+CHECK・`ApplyApproval` set/`Withdraw` clear〔F3 撤回永久失敗を構造封鎖〕・`MonthlySummaries::LeaveAggregator`〔snapshot 優先・全種別 hours・paid 日数・period.range 直読で drift ゼロ〕・MAS 2 列。多視点 6 視点＋tenant-isolation/approval-engine レビュー clean）（PR #14）／**3-3b CSV 出力**（exporter+controller+UI・社員識別子列・formula injection サニタイズ・SPEC §6.4 追記）は後続
