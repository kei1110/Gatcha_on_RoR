...
- [ ] **Phase 2〜3 機能の動線整備（最小グローバルナビ）**: Phase 0a〜3 で実装した機能画面（休暇申請・打刻変更・休日出勤・承認インボックス・月次サマリ/CSV・管理マスタ）が、唯一のランディング（root=home）から**クリック到達不能**（layout にナビ無し・各 path は自ディレクトリ外から参照ゼロ＝URL 直打ち専用）。特に **CCR index に「新規申請」リンクが無い**（LR/HWR には有る）・**残高 CRUD（`admin/leave_balances`）が `users/show` から被リンクゼロ**で機能内断絶。`home/_clocking.html.erb` の「打刻変更申請（Phase 2 で提供予定）」も陳腐化。`Admin::NavComponent` 同型の最小 `GlobalNavComponent` を layout に置き、CCR 新規リンク／残高セクション／文言を併せて修正する（Phase 3 spec-check ユーザーストーリー観点・横断バックログ「デザインシステム整備」のナビ抽出を最小先取り）
