...
RSpec.describe Admin::LeaveTypePolicy, type: :policy do
  subject { described_class.new(actor, record) }

  let(:record) { create(:leave_type) }
  ...
  describe "Scope" do
    it "組織全件（inactive 含む）・他テナント漏れなし" do
      ...
      inactive = create(:leave_type, active: false)
      ActsAsTenant.with_tenant(create(:organization)) { create(:leave_type) }

      resolved = described_class::Scope.new(actor, LeaveType.all).resolve
    ...
    it "without_tenant 文脈でも自組織のみ（organization_id 明示の fail-open 検出 — test_tenant 下では検知不能）" do
      ...
      resolved = ActsAsTenant.without_tenant do
        described_class::Scope.new(actor, LeaveType.all).resolve.to_a
      end
