...
RSpec.describe "db/seeds.rb", type: :model do
  it "2 回実行しても件数不変・例外なし（冪等 + seed 値が全バリデーションを通る検証を兼ねる）" do
    ...
    counts = ActsAsTenant.without_tenant { [ Organization.count, User.count, WorkPattern.count, LeaveType.count, UserWorkPattern.count ] }
    expect { Rails.application.load_seed }.not_to raise_error
    expect(ActsAsTenant.without_tenant { [ Organization.count, User.count, WorkPattern.count, LeaveType.count, UserWorkPattern.count ] }).to eq(counts)
